...
`);class C{static async create({sampleRate:e,format:s,preferHeadphonesForIosDevices:t}){let n=null,o=null;try{const d={sampleRate:{ideal:e},echoCancellation:{ideal:!0},noiseSuppression:{ideal:!0}};if(q()&&t){const l=(await window.navigator.mediaDevices.enumerateDevices()).find(u=>u.kind==="audioinput"&&["airpod","headphone","earphone"].find(p=>u.label.toLowerCase().includes(p)));l&&(d.deviceId={ideal:l.deviceId})}const m=navigator.mediaDevices.getSupportedConstraints().sampleRate;n=new window.AudioContext(m?{sampleRate:e}:{});const v=n.createAnalyser();m||await n.audioWorklet.addModule("https://cdn.jsdelivr.net/npm/@alexanderolsen/libsamplerate-js@2.1.2/dist/libsamplerate.worklet.js"),await $(n.audioWorklet),o=await navigator.mediaDevices.getUserMedia({audio:d});const f=n.createMediaStreamSource(o),a=new AudioWorkletNode(n,"raw-audio-processor");return a.port.postMessage({type:"setFormat",format:s,sampleRate:e}),f.connect(v),v.connect(a),await n.resume(),new C(n,v,a,o)}catch(d){var r,c;throw(r=o)==null||r.getTracks().forEach(m=>m.stop()),(c=n)==null||c.close(),d}}constructor(e,s,t,n){this.context=void 0,this.analyser=void 0,this.worklet=void 0,this.inputStream=void 0,this.context=e,this.analyser=s,this.worklet=t,this.inputStream=n}async close(){this.inputStream.getTracks().forEach(e=>e.stop()),await this.context.close()}setMuted(e){this.worklet.port.postMessage({type:"setMuted",isMuted:e})}}const z=P("audio-concat-processor",`
...
`);class S{static async create({sampleRate:e,format:s}){let t=null;try{t=new AudioContext({sampleRate:e});const o=t.createAnalyser(),r=t.createGain();r.connect(o),o.connect(t.destination),await z(t.audioWorklet);const c=new AudioWorkletNode(t,"audio-concat-processor");return c.port.postMessage({type:"setFormat",format:s}),c.connect(r),await t.resume(),new S(t,o,r,c)}catch(o){var n;throw(n=t)==null||n.close(),o}}constructor(e,s,t,n){this.context=void 0,this.analyser=void 0,this.gain=void 0,this.worklet=void 0,this.context=e,this.analyser=s,this.gain=t,this.worklet=n}async close(){await this.context.close()}}class M extends b{static async startSession(e){var s;const t=b.getFullOptions(e);t.onStatusChange({status:"connecting"}),t.onCanSendFeedbackChange({canSendFeedback:!1});let n=null,o=null,r=null,c=null,d=null;if((s=e.useWakeLock)==null||s)try{d=await navigator.wakeLock.request("screen")}catch{}try{var m;return c=await navigator.mediaDevices.getUserMedia({audio:!0}),await B(t.connectionDelay),o=await j.create(e),[n,r]=await Promise.all([C.create(_({},o.inputFormat,{preferHeadphonesForIosDevices:e.preferHeadphonesForIosDevices})),S.create(o.outputFormat)]),(m=c)==null||m.getTracks().forEach(p=>p.stop()),c=null,new M(t,o,n,r,d)}catch(p){var v,f,a,l;t.onStatusChange({status:"disconnected"}),(v=c)==null||v.getTracks().forEach(y=>y.stop()),(f=o)==null||f.close(),await((a=n)==null?void 0:a.close()),await((l=r)==null?void 0:l.close());try{var u;await((u=d)==null?void 0:u.release()),d=null}catch{}throw p}}constructor(e,s,t,n,o){super(e,s),this.input=void 0,this.output=void 0,this.wakeLock=void 0,this.inputFrequencyData=void 0,this.outputFrequencyData=void 0,this.onInputWorkletMessage=r=>{this.status==="connected"&&this.connection.sendMessage({user_audio_chunk:V(r.data[0].buffer)})},this.onOutputWorkletMessage=({data:r})=>{r.type==="process"&&this.updateMode(r.finished?"listening":"speaking")},this.addAudioBase64Chunk=r=>{this.output.gain.gain.value=this.volume,this.output.worklet.port.postMessage({type:"clearInterrupted"}),this.output.worklet.port.postMessage({type:"buffer",buffer:W(r)})},this.fadeOutAudio=()=>{this.updateMode("listening"),this.output.worklet.port.postMessage({type:"interrupt"}),this.output.gain.gain.exponentialRampToValueAtTime(1e-4,this.output.context.currentTime+2),setTimeout(()=>{this.output.gain.gain.value=this.volume,this.output.worklet.port.postMessage({type:"clearInterrupted"})},2e3)},this.calculateVolume=r=>{if(r.length===0)return 0;let c=0;for(let d=0;d<r.length;d++)c+=r[d]/255;return c/=r.length,c<0?0:c>1?1:c},this.input=t,this.output=n,this.wakeLock=o,this.input.worklet.port.onmessage=this.onInputWorkletMessage,this.output.worklet.port.onmessage=this.onOutputWorkletMessage}async handleEndSession(){await super.handleEndSession();try{var e;await((e=this.wakeLock)==null?void 0:e.release()),this.wakeLock=null}catch{}await this.input.close(),await this.output.close()}handleInterruption(e){super.handleInterruption(e),this.fadeOutAudio()}handleAudio(e){this.lastInterruptTimestamp<=e.audio_event.event_id&&(this.options.onAudio(e.audio_event.audio_base_64),this.addAudioBase64Chunk(e.audio_event.audio_base_64),this.currentEventId=e.audio_event.event_id,this.updateCanSendFeedback(),this.updateMode("speaking"))}setMicMuted(e){this.input.setMuted(e)}getInputByteFrequencyData(){return this.inputFrequencyData!=null||(this.inputFrequencyData=new Uint8Array(this.input.analyser.frequencyBinCount)),this.input.analyser.getByteFrequencyData(this.inputFrequencyData),this.inputFrequencyData}getOutputByteFrequencyData(){return this.outputFrequencyData!=null||(this.outputFrequencyData=new Uint8Array(this.output.analyser.frequencyBinCount)),this.output.analyser.getByteFrequencyData(this.outputFrequencyData),this.outputFrequencyData}getInputVolume(){return this.calculateVolume(this.getInputByteFrequencyData())}getOutputVolume(){return this.calculateVolume(this.getOutputByteFrequencyData())}}class H extends b{static startSession(e){return e.textOnly?N.startSession(e):M.startSession(e)}}function x(){return x=Object.assign?Object.assign.bind():function(i){for(var e=1;e<arguments.length;e++){var s=arguments[e];for(var t in s)({}).hasOwnProperty.call(s,t)&&(i[t]=s[t])}return i},x.apply(null,arguments)}const J=["micMuted","volume"];function G(i={}){const{micMuted:e,volume:s}=i,t=function(a,l){if(a==null)return{};var u={};for(var p in a)if({}.hasOwnProperty.call(a,p)){if(l.includes(p))continue;u[p]=a[p]}return u}(i,J),n=g.useRef(null),o=g.useRef(null),[r,c]=g.useState("disconnected"),[d,m]=g.useState(!1),[v,f]=g.useState("listening");return g.useEffect(()=>{var a;e!==void 0&&(n==null||(a=n.current)==null||a.setMicMuted(e))},[e]),g.useEffect(()=>{var a;s!==void 0&&(n==null||(a=n.current)==null||a.setVolume({volume:s}))},[s]),g.useEffect(()=>()=>{var a;(a=n.current)==null||a.endSession()},[]),{startSession:async a=>{var l;if((l=n.current)!=null&&l.isOpen())return n.current.getId();if(o.current)return(await o.current).getId();try{return o.current=H.startSession(x({},t??{},a??{},{onModeChange:({mode:u})=>{f(u)},onStatusChange:({status:u})=>{c(u)},onCanSendFeedbackChange:({canSendFeedback:u})=>{m(u)}})),n.current=await o.current,e!==void 0&&n.current.setMicMuted(e),s!==void 0&&n.current.setVolume({volume:s}),n.current.getId()}finally{o.current=null}},endSession:async()=>{const a=n.current;n.current=null,await a?.endSession()},setVolume:({volume:a})=>{var l;(l=n.current)==null||l.setVolume({volume:a})},getInputByteFrequencyData:()=>{var a;return(a=n.current)==null?void 0:a.getInputByteFrequencyData()},getOutputByteFrequencyData:()=>{var a;return(a=n.current)==null?void 0:a.getOutputByteFrequencyData()},getInputVolume:()=>{var a,l;return(a=(l=n.current)==null?void 0:l.getInputVolume())!=null?a:0},getOutputVolume:()=>{var a,l;return(a=(l=n.current)==null?void 0:l.getOutputVolume())!=null?a:0},sendFeedback:a=>{var l;(l=n.current)==null||l.sendFeedback(a)},getId:()=>{var a;return(a=n.current)==null?void 0:a.getId()},sendContextualUpdate:a=>{var l;(l=n.current)==null||l.sendContextualUpdate(a)},sendUserMessage:a=>{var l;(l=n.current)==null||l.sendUserMessage(a)},sendUserActivity:()=>{var a;(a=n.current)==null||a.sendUserActivity()},sendMCPToolApprovalResult:(a,l)=>{var u;(u=n.current)==null||u.sendMCPToolApprovalResult(a,l)},status:r,canSendFeedback:d,micMuted:e,isSpeaking:v==="speaking"}}function Z(){const i=G({onConnect:()=>console.log("Connected"),onDisconnect:()=>console.log("Disconnected"),onMessage:t=>console.log("Message:",t),onError:t=>console.error("Error:",t)}),e=g.useCallback(async()=>{try{await navigator.mediaDevices.getUserMedia({audio:!0}),await i.startSession({agentId:"agent_01jz0e2bj6efea9t0pvajxdp79"})}catch(t){console.error("Failed to start conversation:",t)}},[i]),s=g.useCallback(async()=>{await i.endSession()},[i]);return h.jsxs("div",{className:"flex flex-col items-center gap-4",children:[h.jsxs("div",{className:"flex gap-2",children:[h.jsx("button",{onClick:e,disabled:i.status==="connected",className:"px-4 py-2 bg-blue-500 text-white rounded disabled:bg-gray-300",children:"Start Conversation"}),h.jsx("button",{onClick:s,disabled:i.status!=="connected",className:"px-4 py-2 bg-red-500 text-white rounded disabled:bg-gray-300",children:"Stop Conversation"})]}),h.jsxs("div",{className:"flex flex-col items-center",children:[h.jsxs("p",{className:"text-white transition-all duration-500 ease-in-out",children:["Status: ",i.status]}),h.jsxs("p",{className:`text-white transition-all duration-500 ease-in-out ${i.isSpeaking?"animate-pulse":"animate-fadeIn"}`,children:["Agent is ",i.isSpeaking?"speaking":"listening"]})]})]})}function X(){const[i,e]=g.useState(!1);return h.jsxs("div",{className:"fixed bottom-24 right-4 z-50",children:[!i&&h.jsx("button",{onClick:()=>e(!0),className:"bg-primaryColor shadow-xl text-white rounded-full w-16 h-16 flex items-center justify-center text-3xl hover:bg-primaryColorHover transition-all border-4 border-bgDark1","aria-label":"Open voice call widget",children:h.jsxs("svg",{width:"32",height:"32",viewBox:"0 0 24 24",fill:"none",children:[h.jsx("circle",{cx:"12",cy:"12",r:"12",fill:"currentColor",opacity:"0.1"}),h.jsx("path",{d:"M7.5 8.5C8.32843 7.67157 9.67157 7.67157 10.5 8.5L11.5 9.5C12.3284 10.3284 12.3284 11.6716 11.5 12.5L11 13C12.5 15 15 17.5 17 19L17.5 18.5C18.3284 17.6716 19.6716 17.6716 20.5 18.5L21.5 19.5C22.3284 20.3284 22.3284 21.6716 21.5 22.5C19.5 24.5 14.5 19.5 12.5 17.5C10.5 15.5 5.5 10.5 7.5 8.5Z",fill:"currentColor"})]})}),i&&h.jsxs("div",{className:"relative bg-bgDark1 border border-primaryColor rounded-2xl shadow-2xl p-6 min-w-[320px] max-w-[90vw]",children:[h.jsxs("div",{className:"flex items-center justify-between mb-4",children:[h.jsx("span",{className:"font-bold text-primaryColor text-lg",children:"Voice Assistant"}),h.jsx("button",{onClick:()=>e(!1),className:"text-2xl text-primaryColor hover:text-primaryColorHover transition","aria-label":"Close voice call widget",children:"×"})]}),h.jsx(Z,{})]})]})}export{X as VoiceWidget};
